...
function ProjectSourceLink({ project }: { project: (typeof projects)[number] }) {
  const label = "githubLabel" in project ? project.githubLabel : "Source code";
  return <OutsideLink href={project.github} label={`View ${project.title} ${label.toLowerCase()} on GitHub`}><Github size={15} aria-hidden="true" />{label}<ArrowUpRight size={13} aria-hidden="true" /></OutsideLink>;
}
